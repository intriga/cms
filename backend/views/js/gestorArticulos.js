// Agregar Articulo
$("#btnagregarArtículo").click(function() {
  $("#agregarArtículo").toggle(400);
});


//subir imagen a travel del input
$("#subirFoto").change(function() {
  ...
  if (imagenType == "image/jpeg" || imagenType == "image/jpg" || imagenType == "image/png") {
    ...
    $("#arrastreImagenArticulo").before('<div class="alert alert-warning alerta text-center">El archivo debe ser JPG o PNG</div>');
  }

  //Mostrar imagen con ajax
  if (Number(imagenSize) < 2000000 && imagenType == "image/jpeg" || imagenType == "image/jpg" || imagenType == "image/png") {
...
$(".editarArticulo").click(function(){
  ...
  $("#"+idArticulo).html('<form method="post" enctype="multipart/form-data">'+
                        '<span>' +
                          '<input type="submit" style="width:10%; padding:5px 0;" class="btn btn-primary pull-right" value="Guardar">' +
                          '</span>' +

                          '<div id="editarImagen"><input style="display:none" type="file" id="subirNuevaFoto" class="btn btn-default"><div id="nuevaFoto"><span class="fa fa-times cambiarImagen"></span><img src="'+rutaimagen+'" class="img-thumbnail"></div></div>' +

                          '<input type="text" value="'+titulo+'" name="editarTitulo">' +

                          '<textarea cols="30" rows="5" name="editarIntroduccion">'+introduccion+'</textarea>' +

                          '<textarea name="editarContenido" id="editarContenido" cols="30" rows="10">'+contenido+'</textarea>' +

                          '<input type="hidden" value="'+idArticulo+'" name="id">' +
  ...
  $(".cambiarImagen").click(function() {
    ...
    $("#subirNuevaFoto").change(function(){
      ...
      if (imagenType == "image/jpeg" || imagenType == "image/jpg" || imagenType == "image/png") {
        ...
        $("#editarImagen").before('<div class="alert alert-warning alerta text-center">El archivo debe ser JPG o PNG</div>');
      }
      ...
      if (Number(imagenSize) < 2000000 && imagenType == "image/jpeg" || imagenType == "image/jpg" || imagenType == "image/png") {
        ...
        $.ajax({
          ...
          processData: false,
          beforeSend: function(){
            $("#nuevaFoto").html('<img src="views/images/status.gif" style="width:25%" id="status2">');
          },
          success: function(respuesta){
            $("#status2").remove();
            ...
            if (respuesta == 0) {
              ...
              $("#nuevaFoto").html('<div><img src="'+respuesta.slice(6)+'" class="img-thumbnail"></div>');
            }
