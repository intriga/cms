...
$("#columnasSlide").on("drop", function(e) {
  ...
  if (Number(imagenSize) > 2000000) {
    $("#columnasSlide").before('<div class="alert alert-warning alerta text-center">El archivo excede los 2MB</div>');
  }
  else{
    $(".alerta").remove();
  }
  ...
  if (imagenType == "image/jpeg" || imagenType == "image/jpg" || imagenType == "image/png") {
    ...
    $("#columnasSlide").before('<div class="alert alert-warning alerta text-center">El archivo debe ser JPG o PNG</div>');
  }
      ...
      success: function(respuesta){
        ...
        if (respuesta == 0) {
          ...
          $("#columnasSlide").append('<li class="bloqueSlide">' +
              ...
              '<img src="'+respuesta["ruta"].slice(6)+'" class="handleImg">' +
            '</li>');

          $("#ordenarTextSlide").append('<li>' +
                                        ...
                                        '</li>');
          //window.location.reload(); sin swal
            swal({
              title: "¡OK!",
...
$(".editarSlide").click(function() {
  ...
  $(this).parent().html('<img src="'+rutaImagen+'" class="img-thumbnail">' +
    '<input type="text" class="form-control" id="enviarTitulo" placeholder="Título" value="'+rutaTitulo+'">' +
    '<textarea row="5" id="enviarDescripcion" class="form-control" placeholder="Descripción">'+rutaDescripcion+'</textarea>' +
    ...
    $("#guardar"+idSlide).click(function() {
      ...
      $.ajax({
        ...
        dataType: "json",
        success: function(respuesta) {
          $("#guardar"+idSlide).parent().html('<span class="fa fa-pencil editarSlide" style="background:blue"></span>' +
                                              '<img src="'+rutaImagen+'" style="float:left; margin-bottom:10px" width="80%">' +
                                              '<h1>'+respuesta["titulo"]+'</h1>' +
                                              '<p>'+respuesta["descripcion"]+'</p>');
  ...
  $("#columnasSlide").sortable({
    ...
    stop: function(event){
      for (var i = 0; i < $("#columnasSlide li").length; i++) {
        amacenarOrdenId[i] = event.target.children[i].id;
        ordenItem[i] = i+1;
      }
    }
  })
...
$("#guardarSlide").click(function() {
  ...
  for (var i = 0; i < $("#columnasSlide li").length; i++) {
    ...
    actualizarOrden.append('actualizarOrdenSlide', amacenarOrdenId[i]);
